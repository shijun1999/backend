$(document).ready(function () {
    ...
    $('#fuelux-wizard-container')
        ...
        .on('finished.fu.wizard', function (e) {
            if ($(".progress-bar-success").size() == 0 && $("#uploaded-images").children().size() == 0) {
                ...
                var uploadedImagesArray = new Array();
                for (var i = 0; i < $("#uploaded-images").children().size(); i++) {
                    uploadedImagesArray.push($("#uploaded-images").children().eq(i).attr('id'));
                }
                obj.uploadedCoursePictureIdentiNoList = uploadedImagesArray;


                $.ajax({
                    url: 'localhostCourseCloneAjax/cloneCourse',
                    type: 'post',
                    dataType: 'json',
    ...
    $.ajax({
        url: 'localhostCourseCloneAjax/cloneCourseInit',
        type: 'post',
        dataType: 'json',
        data: {
            courseIdentiNo: getUrlParam('courseIdentiNo')
