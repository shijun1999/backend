...
public class LocalhostChatAjaxAction extends LocalhostMessageModel {
	...
	public String fetchChat() {
		...
		try {
			...
			KootourMessage retMsg = localhostMessageService.fetchChat(inParamMap);
	...
	public String sendMessage() {
		...
		try {
			...
			Map<String, Object> inParamMap = new HashMap<String, Object> ();
			inParamMap.put(KooConst.LT_IN_PARAM_MODEL_KEY, this);
			KootourMessage retMsg = localhostMessageService.sendMessage(inParamMap);
			if (retMsg.isHasError()) {
				jsonMap.put("result", "error");
				jsonMap.put("message", retMsg.getMessage());
				jsonStr = gson.toJson(jsonMap);
